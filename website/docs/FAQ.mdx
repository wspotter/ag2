...
- [`max_consecutive_auto_reply`](/reference/autogen/ConversableAgent#max-consecutive-auto-reply-2) the maximum number of consecutive auto replie (a reply from an agent without human input is considered an auto reply). It plays a role when `human_input_mode` is not "ALWAYS".
- [`max_turns` in `ConversableAgent.initiate_chat`](/reference/autogen/ConversableAgent#initiate-chat) limits the number of conversation turns between two conversable agents (without differentiating auto-reply and reply/input from human)
- [`max_round` in GroupChat](https://docs.ag2.ai/reference/autogen/GroupChat#groupchat) specifies the maximum number of rounds in a group chat session.
...
to install and run AG2 in docker.
...
Please refer to [here](/reference/autogen/ConversableAgent#chat-messages)
