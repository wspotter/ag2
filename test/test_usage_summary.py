...
from autogen.usage_summary import create_usage_summary_model, _change_usage_summary_format, UsageSummary, ActualUsageSummary, TotalUsageSummary, ModelUsageSummary
...
@pytest.mark.parametrize(
    ...
    [
        (
            {'gpt-4o-mini-2024-07-18': {'completion_tokens': 25, 'cost': 4.23e-05, 'prompt_tokens': 182, 'total_tokens': 207}, 'total_cost': 4.23e-05},
            {'gpt-4o-mini-2024-07-18': {'completion_tokens': 25, 'cost': 4.23e-05, 'prompt_tokens': 182, 'total_tokens': 207}, 'total_cost': 4.23e-05},
            {
                "actual": {
                    "usages": [
...
def test_usage_summary_print_same_actual_and_total(
    ...
    expected_call_args_list = [call('----------------------------------------------------------------------------------------------------', flush=True),
 call('Usage summary excluding cached usage: ', flush=True),
 call('Total cost: 4e-05', flush=True),
 call("* Model 'gpt-4o-mini-2024-07-18': cost: 4e-05, prompt_tokens: 182, completion_tokens: 25, total_tokens: 207", flush=True),
 call(),
 call('All completions are non-cached: the total cost with cached completions is the same as actual cost.', flush=True),
 call('----------------------------------------------------------------------------------------------------', flush=True)]
...
@pytest.mark.parametrize(
    ...
    [
        (
            {'gpt-4o-mini-2024-07-18': {'completion_tokens': 25, 'cost': 4.23e-05, 'prompt_tokens': 182, 'total_tokens': 207}, 'total_cost': 4.23e-05},
            {'gpt-4o-mini-2024-07-18': {'completion_tokens': 25*40, 'cost': 4.23e-05*40, 'prompt_tokens': 182*40, 'total_tokens': 207*40}, 'total_cost': 4.23e-05*40},
...
def test_usage_summary_print_different_actual_and_total(
 ...
 call("* Model 'gpt-4o-mini-2024-07-18': cost: 0.00169, prompt_tokens: 7280, completion_tokens: 1000, total_tokens: 8280", flush=True),
 call('----------------------------------------------------------------------------------------------------', flush=True)]
...
def test_usage_summary_print_none_actual_and_total(
    ...
    assert actual.actual.usages == None
    assert actual.actual.total_cost == None

    assert actual.total.usages == None
    assert actual.total.total_cost == None
